...
\item shp file suitable for GIS softwares
...
SHP file output is provided by classes: 
\begin{itemize}
\item
  \verb|shp.SHPAreaWriter|
\item
  \verb|shp.SHPPointWriter|
\end{itemize}
The first class creates polygon shp output with area and line emissions before time dissagregation in mol/s (g/s) as average year emission. The second class creates point shp output with point sources.  An example of the postprocessing section in the FUME configuration for SHP output is:

\begin{verbatim}
[postproc]
    processors = postproc.shp.SHPAreaWriter, postproc.shp.SHPPointWriter
    
    [[shpwriter]]
      outfile_area = area.shp
      outfile_point = point.shp
\end{verbatim}
