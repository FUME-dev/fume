...
BEGIN
    ...
    EXECUTE format('DROP TABLE IF EXISTS %I.ep_time_var_mapping_all', case_schema);
    EXECUTE format('CREATE TABLE %I.ep_time_var_mapping_all AS SELECT cat_id, tv_id FROM %I.ep_time_var_mapping', case_schema, conf_schema);
    EXECUTE format('ALTER TABLE %I.ep_time_var_mapping_all ADD PRIMARY KEY (cat_id, tv_id)', case_schema);
    ...
    time_cur := time_start;
    raise notice 'time_cur %', time_cur;
    WHILE time_step < num_times
    LOOP
        -- insert timezone shifts for given UTC time
        time_out = time_cur;-- AT TIME ZONE tzone_out;
        raise notice 'time_out %', time_out;
        FOR tz_id, tz_name IN EXECUTE format('SELECT tz_id, tz_name FROM %I.ep_timezones',case_schema)
        LOOP
            time_loc = time_out AT TIME ZONE tz_name;
            raise notice 'time_loc %', time_loc;
            EXECUTE format('INSERT INTO %I.ep_time_zone_shifts (tz_id, time_loc, time_out) VALUES (%L,%L,%L)', case_schema, tz_id, time_loc, time_cur);
        END LOOP;

        time_step := time_step + 1;
        time_cur := time_cur + time_increment;
    END LOOP;
    -- recompile statistics
    ...
    EXECUTE format('UPDATE %I.ep_time_factors AS oldvals
                    SET tv_factor = newvals.tv_factor
                    FROM %I.ep_time_var_series AS newvals
                    WHERE (newvals.cat_id, newvals.time_loc) = (oldvals.cat_id, oldvals.time_loc)', case_schema, conf_schema);
    -- recompile statistics
...
BEGIN
    ...
    execute format('INSERT INTO %I.ep_sp_factors (cat_id, spec_in_id, spec_sp_id, split_factor)
                SELECT cat_id, spec_in_id, spec_sp_id, mole_split_factor/mol_weight as split_factor
    ...
    execute format('INSERT INTO %I.ep_mod_spec_factors (cat_id, spec_in_id, spec_mod_id, split_factor)
                SELECT cat_id, spec_in_id, spec_id, new_split_factor FROM (
                    SELECT cat_id, spec_in_id,
                        COALESCE(spec_mod_name, sp_nam.name) as mod_name,
                        COALESCE(map_fact*split_factor, split_factor) as new_split_factor
...
BEGIN
    ...
    EXECUTE format('DROP TABLE IF EXISTS %I.ep_mod_spec_factors_all', case_schema);
    EXECUTE format('CREATE TABLE %I.ep_mod_spec_factors_all AS SELECT cat_id, spec_in_id, spec_mod_id, split_factor FROM %I.ep_mod_spec_factors', case_schema, case_schema);
    EXECUTE format('ALTER TABLE %I.ep_mod_spec_factors_all ADD PRIMARY KEY (cat_id, spec_in_id, spec_mod_id)', case_schema);
    FOR spec_in_id IN EXECUTE format('SELECT distinct spec_in_id FROM %I.ep_mod_spec_factors', case_schema) LOOP
        FOR cat_id IN EXECUTE format('SELECT c.cat_id FROM %I.ep_emission_categories c LEFT JOIN (SELECT cat_id FROM %I.ep_mod_spec_factors WHERE spec_in_id=$1) s USING(cat_id) WHERE s.cat_id IS NULL', conf_schema, case_schema) USING(spec_in_id) LOOP
            RAISE NOTICE '%', cat_id || ' '|| spec_in_id;
...
---            RAISE NOTICE '%', format('INSERT INTO %I.ep_mod_spec_factors SELECT %s, split_factor from %I.ep_mod_spec_factors WHERE cat_id=%s AND spec_in_id=%s', case_schema, cat_id, case_schema, par_id, spec_in_id);
            EXECUTE format('INSERT INTO %I.ep_mod_spec_factors_all SELECT $1, spec_in_id, spec_mod_id, split_factor from %I.ep_mod_spec_factors WHERE cat_id=$2 AND spec_in_id=$3', case_schema, case_schema) USING cat_id, par_id, spec_in_id;
            RAISE NOTICE '%', par_id;
        END LOOP;
    END LOOP;
    -- recompile statistics
...
BEGIN
    ...
    EXECUTE format('INSERT INTO %I.ep_sg_emissions (sg_id, spec_in_id, cat_id, emiss)
                    SELECT sgs.sg_id, ie.spec_in_id, ie.cat_id, ie.emission*sgs.sg_factor
                    ...
                    case_schema, case_schema, source_schema, emissions_table, source_schema, sources_table, source_schema, eset_table);
...
BEGIN
    ...
    q = 'INSERT INTO %I.ep_sources_grid (source_type, source_id, grid_id, k, sg_factor)
                    SELECT s.source_type, s.source_id, f.grid_id, 1, f.%I
                           FROM %I.%I s
