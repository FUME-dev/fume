...
def combine_emis(em1,spec1,em2,spec2): # function to combine two emission arrays 
    ...
    for s in spec2:
        if s not in spec1:
            spec.append(s)
    numspec = len(spec)
    emisout = np.zero((kx,ky,numspec),dtype=float)
...
#            point_src_params = emiscur.fetchall()
    ...
    ######################################################################################
    # get data from external models:
    # MEGAN
    if 'megan' in ep_cfg.run_params.models.models:
        meganfile = os.path.join(ep_cfg.megan_out_dir,ep_cfg.megan_out)

        m = Dataset(meganfile,'r')
        megan_species = getattr(m,'VAR-LIST').split().remove('GDAY')
        numspec_megan = len(megan_species)
        ep_species    = list()
        ep_species    = species[:]
    # extend ep_species by megan_species
...
#        emissions = np.arange(nx*ny*numvar*ntimeint).reshape((nx,ny,numvar,ntimeint),order='F')
    ...
    #   end of temporaty data
        ...
        for t in range(ntimeint):
            ...
            emis_ep = np.sum(emis, axis=2) # in CAMx, we do not have elevated emissions (3D emissions), so sum up to the ground
            
            if 'megan' in ep_cfg.run_params.models.models:
                        
                emis_megan = np.zero((nx,ny,numspec_megan),dtype=float)
                for i in range(numspec_megan):
                    emis_megan[:,:,i] = m.variables[species_megan[i]][t,0,:].transpose(1,0)
            ...
            for i in range(numspec):
                ...
                data = emisout[:,:,i].flatten('F')*tdelta # mol/timeinterval
