...
from lib.ep_libutil import ep_create_grid, ep_create_schema, ep_projection_params, \
    ...
    ep_dates_times, ep_internal_path, ep_grid_params_to_rtcfg, is_regular_grid
...
def create_new_case():
    ...
    if ep_cfg.domain.create_grid:
        ep_create_grid()
    # user-supplied geometry needs to be copied from source geometries to conf schema
    else:
        ep_read_domain_from_shp()
...
def ep_create_timezones():
    ...
    try:
        cur.execute('select ep_case_timezones(%s,%s,%s,%s,%s,%s,%s,%s,%s)', ( \
            ep_cfg.db_connection.conf_schema, ep_cfg.domain.grid_name, ep_cfg.db_connection.static_schema,
            'ep_tz_world',
            ep_cfg.db_connection.case_schema, 'ep_timezones',
            'ep_grid_env', ep_cfg.domain.grid_timezone,
            ep_cfg.projection_params.projection_srid))
        log.sql_info(ep_connection)
    except Exception as e:
...
def ep_read_domain_from_shp():
    """
    Reads shapefile which defines the computational domain.
    It also tries to add i,j,xmin,ymin,xmax,ymax parameters to the user supplied geometry - which may or may not be
    a regular grid.
    """
    conf_schema = ep_cfg.db_connection.conf_schema
    shpfile = ep_cfg.domain.domain_shp
    grid_name = ep_cfg.domain.grid_name
    srid = ep_cfg.projection_params.projection_srid

    cur = ep_connection.cursor()

    ep_shp2postgis(shpfile, conn=ep_connection,
                   schema=conf_schema, tablename='domain_shp_import')

    cur.callproc('ep_create_empty_gridtable', [conf_schema, grid_name, srid])

    cur.execute('insert into "{conf_schema}"."{}" (geom) '
                "select ST_Transform(geom, %s) as geom "
                'from "{conf_schema}".domain_shp_import '.format(grid_name,
                                conf_schema=conf_schema),  (srid, ))
    cur.execute('create index if not exists "{}" on "{conf_schema}"."{}" using gist(geom)'
                .format(grid_name + '_geom', grid_name,  conf_schema=conf_schema))
    cur.execute('UPDATE "{conf_schema}"."{}" '
                'set xmi = round(ST_xmin(geom)::numeric, 7), '
                'ymi = round(ST_ymin(geom)::numeric, 7), '
                'xma = round(ST_xmax(geom)::numeric, 7), '
                'yma = round(ST_ymax(geom)::numeric, 7)'.format(grid_name, conf_schema=conf_schema))
    ep_connection.commit()

    if is_regular_grid(conf_schema, grid_name):
        # geometry is regular grid
        cur.execute('select count(distinct xmi), count(distinct ymi), '
                    'min(xmi), min(ymi), '
                    'max(xma), max(yma) from "{conf_schema}"."{}"'
                    .format(grid_name, conf_schema=conf_schema))
        nxmin, nymin, xmin, ymin, xmax, ymax = cur.fetchone()
        delx = (xmax - xmin) / nxmin
        dely = (ymax - ymin) / nymin
        cur.execute('update "{conf_schema}"."{}" '
                    'set i = round((xmi - %s) / %s) + 1, j = round((ymi - %s) / %s) + 1'.
                    format(grid_name, conf_schema=conf_schema), (xmin, delx, ymin, dely))
        cur.execute('create index if not exists "{}" on "{conf_schema}"."{}" (i,j)'
                    .format(grid_name + '_i_j', grid_name, conf_schema=conf_schema))
        ep_connection.commit()
    cur.close()

    ep_grid_params_to_rtcfg()
...
def ep_create_grid_tz():
    ...
    conf_schema = ep_cfg.db_connection.conf_schema
    grid_name = ep_cfg.domain.grid_name
    log.debug('Create grid with timezones: ', case_schema, 'ep_grid_tz', conf_schema, grid_name, 'ep_timezones',
              ep_cfg.projection_params.projection_srid)
    cur = ep_connection.cursor()
    try:
        cur.callproc('ep_create_grid_tz', [case_schema, 'ep_grid_tz', conf_schema, grid_name, 'ep_timezones',
                                           ep_cfg.projection_params.projection_srid])
        cur.close()
        ep_connection.commit()
    except Exception as e:
...
def prepare_conf():
    ...
    to be processed. It actually reads/creates projection from params
    and creates original grid in conf schema if required
    by configuration. More action might be added in the future.
    """

    # First, check if the function was already run and skip if it was.
    if ep_rtcfg['conf_prepared']:
        log.debug('prepare_conf already run, skipping...')
        return

    log.debug('Running prepare_conf...')
    ...
    report.record.sql('Coordinate system name',
                      'SELECT substring(srtext from \'"(.*?)"\') FROM spatial_ref_sys WHERE srid = {}',
                      ep_rtcfg['projection_params']['srid'])
    # ep_create_grid()
    ep_grid_params_to_rtcfg()
    ...
    # create vertical structure of the output model
    ep_rtcfg.model_levels_stag = np.zeros(ep_cfg.domain.nz, dtype=float)
    ep_rtcfg.model_delz = np.zeros(ep_cfg.domain.nz, dtype=float)
    if ep_cfg.run_params.output_params.model_levels == []:
        # output vertical structure model_levels is not prescribed
        # we generate the structure of the model_levels from parameters
        if ep_cfg.domain.delz == 0:
            ep_rtcfg.delz = ep_cfg.domain.delx
        else:
            ep_rtcfg.delz = ep_cfg.domain.delz
        ep_rtcfg.delz_stretching = (ep_cfg.domain.delz_stretch_factor != 1.0)
        # fill out layer heights and heights of the centres/edges of the levels
        ep_rtcfg.model_levels = np.zeros(ep_cfg.domain.nz, dtype=float)
        dzs = ep_rtcfg.delz
        ep_rtcfg.model_delz[0] = dzs
        ep_rtcfg.model_levels[0] = dzs*0.5
        ep_rtcfg.model_levels_stag[0] = dzs
        for i in range(1,ep_cfg.domain.nz):
            ep_rtcfg.model_delz[i] = dzs
            ep_rtcfg.model_levels_stag[i] = ep_rtcfg.model_levels_stag[i-1] + dzs
            ep_rtcfg.model_levels[i] = ep_rtcfg.model_levels_stag[i-1] + dzs * 0.5
            if ep_rtcfg.delz_stretching and ep_rtcfg.model_levels[i] + dzs >= ep_cfg.domain.delz_stretch_level:
                dzs = min(dzs * ep_cfg.domain.delz_stretch_factor, ep_cfg.domain.delz_max)
    else:
        # output vertical structure model_levels is prescribed
        ep_rtcfg.model_levels = ep_cfg.run_params.output_params.model_levels
        ep_rtcfg.model_levels_stag[0] = ep_rtcfg.model_levels[0] * 2.
        ep_rtcfg.model_delz[0] = ep_rtcfg.model_levels_stag[0]
        for i in range(1,ep_cfg.domain.nz):
            ep_rtcfg.model_levels_stag[i] = ep_rtcfg.model_levels_stag[i-1] + \
                                            2. * (ep_rtcfg.model_levels[i] - ep_rtcfg.model_levels_stag[i-1])
            ep_rtcfg.model_delz[i] = ep_rtcfg.model_levels_stag[i] - ep_rtcfg.model_levels_stag[i-1]
...
def collect_meteorology():
    ...
    The list of meteorology fields is saved in the netcdf file ep_cfg.met_netcdf_file
    """
...
def process_case_speciation_factors():
    """
    ...
    report.sum.sql(
        'Control sums after chemical speciations - emission per specie and eset [gas unit Mmol/year, aerosol unit t/year (depends on gspro file)]',
...
def process_case_time_factors():
    """
    Precalculate time disaggregation factors.
    Will be applied in the postprocessing phase (file output).
    """
...
def process_vertical_distributions():
    ...
    if ep_cfg.run_params.vdistribution_params.apply_vdistribution == False:
        log.debug(
            'Vertical distributions are not applied (cfg -> run_params -> vdistribution_params -> apply_vdistribution set to "no"!')
        return
    ...
    model_levels = ep_rtcfg.model_levels
    nz = ep_cfg.domain.nz
    ...
    with ep_connection.cursor() as cur:
        ...
        for vd in vdists:
            cur.execute(
                'SELECT  cat_id, level, height, factor FROM "{schema}"."ep_vdistribution_factors" WHERE vdistribution_id = %s'
            ...
            # vd_factors = np.zeros((nz, len(categories)))  # unused
            for c in categories:
                model_levels_m = np.zeros((num_levels_m))
                vd_c = [e for e in vdist_data if e[0] == c]
                # numlevels = len(vd_c)  # unused
                ...
                if factor_out > 1e-6:
                    cur.execute(
                        'INSERT INTO "{schema}"."ep_vdistribution_factors_out" (vdistribution_id,cat_id,level,factor) VALUES (%s, %s, %s, %s)'.format(
                            schema=case_schema), (vd, c, 0, factor_out))
                ...
                for i in range(1, nz):
                    ...
                    if factor_out > 1e-6:
                        cur.execute(
                            'INSERT INTO "{schema}"."ep_vdistribution_factors_out" (vdistribution_id,cat_id,level,factor) VALUES (%s, %s, %s, %s)'.format(
                                schema=case_schema), (vd, c, i, factor_out))
...
def preproc_external_models():
    ...
    for mod in ep_rtcfg['external_models']:
        ...
        try:
            ...
            log.fmt_error(
                'EE: Error preprocessing the model {}. Check if it is correctly configured and have all the neccesary inputs',
                mod['name'])
            raise
...
def run_external_models():
    ...
    for mod in ep_rtcfg['external_models']:
        ...
        try:
            ...
            log.fmt_error(
                'EE: Error running model {}. Check if it is correctly configured and have all the neccesary inputs',
                mod['name'])
            raise
...
def create_point_params_table(con, source_schema, case_schema, csrid, ssrid):
    ...
    cur.execute(
        'INSERT INTO "{case_schema}"."ep_sources_point" (sg_id, xstk, ystk, lon, lat, height, diameter, temperature, velocity) '
        'SELECT grid.sg_id, '
...
def fill_missing_point_parameters(con, source_schema, case_schema, conf_schema):
    ...
    cur.execute(
        'INSERT INTO "{case_schema}"."ep_default_point_params" (cat_id, spec_in_id, lim, height, temperature, velocity, diameter) '
    ...
    cur.execute('SELECT * FROM "{case_schema}"."ep_default_point_params_all" order by cat_id, lim DESC'.format(
        case_schema=case_schema))
    rows = cur.fetchall()
    for row in rows:
        category = row[0]
